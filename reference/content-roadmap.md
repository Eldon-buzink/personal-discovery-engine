...
**Deploy-cache bug found and fixed (2026-09-01):** A status check found `/blog`'s sitemap.xml and several publishDate-gated posts serving stale cached responses days after their unlock date passed (why-unhappy-successful-job: cached 404 with `age: 580265`s, ~6.7 days, despite its 2026-08-30 publishDate having passed). This is the same root cause as the earlier Aug 9 post issue (`738d33e`) — Vercel's page/route cache doesn't self-invalidate once a `publishDate` passes; a fresh deploy is required each time to pick it up. Fixed via an empty-commit redeploy (`bf3eddb`). **Standing implication: every future publishDate-gated post likely needs a manual redeploy on or after its unlock date** — this isn't a one-off, it's a property of the current caching setup. Worth fixing at the infra level (e.g. `revalidate` on the blog routes) rather than continuing to patch per-post.
