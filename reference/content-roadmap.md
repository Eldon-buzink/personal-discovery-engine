...
| 2 | Done | Done | Skipped (published on request, no human read-through pass logged) | holland-code-riasec-explained: 2026-08-19 (live). big-five-vs-mbti: 2026-08-24 (live). attachment-style-quiz: 2026-08-29 (live, confirmed 2026-09-01 after cache fix below). |
| 3 | Published (redefined, staggered) | Done | Product-first/traceability check by Claude Code (2026-08-25) — each claim tied to Bearing's scoring logic checked against the actual source (generatePatternCopy.ts, energyScoring.ts, directionScoring.ts); user approved publish on that basis. Not a full manual line-edit pass. | ecr-r-questionnaire-explained: 2026-08-25 (live). why-unhappy-successful-job: 2026-08-30 (live, confirmed 2026-09-01 after cache fix below). why-ambitious-people-cant-name-what-they-want: 2026-09-04 (scheduled via publishDate, not yet live) — new audience-specific post, swapped in for ipip-neo-120-explained. ipip-neo-120-explained, self-efficacy, achievement-striving remain `published: false`, no publishDate set — not part of this batch. |
| 4 | Published (staggered) | Done | Traceability check by Claude Code (2026-09-03) — Working Style claims (21 items, 7/axis, continuous 0-1 position) checked against workingStyleScoring.ts; people-pleasing/routine facet claims checked against the already-published Modesty/Assertiveness/Self-Discipline/Cautiousness posts. Search demand confirmed via live web search for both problem-aware posts. cant-stick-to-a-routine restructured (scenario-led, not myth-led) after it was flagged as sharing an identical skeleton with people-pleasing-cant-say-no — batch now has 4 distinct structures. Not a full manual line-edit pass. | people-pleasing-cant-say-no: 2026-09-03 (live). ideal-work-environment-quiz: 2026-09-08 (scheduled via publishDate). cant-stick-to-a-routine: 2026-09-13 (scheduled via publishDate). personality-test-that-gives-you-a-plan: 2026-09-18 (scheduled via publishDate). |
...
**Deploy-cache bug found and fixed (2026-09-01):** A status check found `/blog`'s sitemap.xml and several publishDate-gated posts serving stale cached responses days after their unlock date passed (why-unhappy-successful-job: cached 404 with `age: 580265`s, ~6.7 days, despite its 2026-08-30 publishDate having passed). Root cause: `app/(site)/blog/[slug]/page.tsx`, `app/(site)/blog/page.tsx`, and `app/sitemap.ts` were all fully statically cached (no `revalidate` set) — a not-yet-published slug requested before its date got dynamically rendered, 404'd, and that 404 was cached *indefinitely* under Next.js's default Full Route Cache, surviving past the publishDate until the next full deploy. Same mechanism as the earlier Aug 9 fix (`738d33e`), confirmed as a recurring bug rather than a one-off.

**Fixed at the infra level (2026-09-01, `db2a518`):** added `export const revalidate = 3600` to all three routes. Scheduled posts, the blog index, and the sitemap now self-resolve within an hour of their `publishDate` — **manual redeploys to unlock a scheduled post are no longer needed going forward.** Verified live post-deploy (fresh `age: 8` header, all Batch 1–3 posts still correctly live/gated).
