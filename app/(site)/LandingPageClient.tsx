...
const landingCSS = `
  ...
  /* HeroBlobs renders a fixed 520x540 canvas (imperative SVG + label divs,
     all sized in raw px) with no responsive logic of its own — on a narrow
     viewport it overflowed well past the edge. Rather than rewrite its
     coordinate system, scale the whole thing down as a unit: the inner ref
     div (SVG + labels together) gets transform:scale(), and the outer
     .hero-blob-scale wrapper is sized to match the scaled footprint so nothing
     else has to account for the transform (transform doesn't affect layout
     box size on its own). Scale factor keeps center/orbit label text and
     spacing all correctly proportional since it's one shared transform.
  */
  @media(max-width:900px){
    .hero-blob-scale{width:338px;height:351px;}
    .hero-blob-scale>div{transform:scale(0.65);transform-origin:top left;}
  }
  @media(max-width:480px){
    .hero-blob-scale{width:260px;height:270px;}
    .hero-blob-scale>div{transform:scale(0.5);transform-origin:top left;}
  }
`
...
function HeroBlobs() {
  ...
  return (
    <div className="hero-blob-scale" style={{ flexShrink:0 }}>
      <div ref={ref} style={{ position:'relative', width:520, height:540 }} />
    </div>
  )
}
      ...
      <div style={{ background:cream, color:charcoal, fontFamily:sans }}>
                ...
                <div>
                  ...
                  {emailSent ? (
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      ...
                      <p style={{ fontSize:14, color:c80, lineHeight:1.6, margin:0 }}>Check your email for the sign-in link.</p>
                    </div>
                  ) : (
        ...
        <section style={{ background:charcoal, color:cream, padding:'80px 48px', position:'relative', overflow:'hidden' }}>
          ...
          <div className="facts-inner" style={{ maxWidth:1100, margin:'0 auto', position:'relative', zIndex:2 }}>
            ...
            <div className="facts-grid" style={{ display:'grid', gap:48, position:'relative', zIndex:2 }}>
              {[
